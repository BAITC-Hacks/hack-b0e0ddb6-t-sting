...
- `apps/web/src/features`: конструктор, разбор, реестр и обучение (`onboarding`). Браузер не реализует формулу Score; переключатель лага считает только учебный пример.
